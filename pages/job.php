<?php 
    ...
    case "regenerate":{
      ...
      foreach ($tabs as $tab) {
        if(file_exists("reports/$tab.$id...rpt")) unlink("reports/$tab.$id...rpt");
      }  */
      $id = is('id', false);
      $int = is('i', 1);
      $t = is('t', 'sentiment');
      if(file_exists("reports/$t.$id.$int.rpt")) unlink("reports/$t.$id.$int.rpt");
      redir("?f=view&id=$id&t=$t&i=$int&".now());
    }
		case "view": {
