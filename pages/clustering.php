...
$content = "<div class='col-md-12'><form method='get'><input type='hidden' name='id' value='{$_GET['id']}' />Distance <input name='e' value='$epsilon' /> Minpoints <input name='m' value='$minpoints' /> <input type='submit' /></form></div>";
...
/*foreach ($singletones as $tweet) {
	$index = find(array_shift($singletones), $tweet, $index);
}*/
...
foreach ($clusters as $cluster) {
	//Whether cluster count has minpoints defined
	if(count($cluster)>=$minpoints){
		$cluster_details .= "Cluser $i: ".count($cluster)."<br />";
		$i++;
		//$x = rand(30,450);
		//$y = rand(30,350);
	...
	//END //Whether cluster count has minpoints defined
}
//Foreach 2:
foreach ($singletones as $tweet) {
...
foreach ($clusters[0] as $tweet_id => $tweet) {
	$index = find($clusters, $tweet_id, $tweet, $index);
	$cur_item++;
}

function find($oclusters, $otweet_id, $otweet, $index){
	global $epsilon;
	global $cur_item;
	$found = false;
